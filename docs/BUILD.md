...
*Companion documents: [DECISION.md](DECISION.md) for the case in twelve minutes, [RESEARCH.md](RESEARCH.md) for the evidence and sources, and **[SPEC-V1.md](SPEC-V1.md)** for the first pathway specified end to end — that is the one an engineer starts from.*

---

## 1. The short answer on data

**Yes, we assume the patient history — and that is fine, as long as we are loud about it.**
...
2. **Guideline-grounded case generation.** We take the real PNPK/PPK documents and generate cases *from* them — so every synthetic patient has a known-correct answer we can score against. This gives us a gold standard on day one, which is the thing we would otherwise have to buy from a panel of doctors.
3. **Deliberately broken cases.** For every clean case we generate a corrupted twin — wrong drug, missed red flag, dose out by 10x, a contraindication ignored. These are the test set for the safety gate. If the gate cannot catch errors we planted ourselves, it will not catch real ones.
...
> after that, with the pack version in the id so editing a guideline costs them
...
- **`interrupt()` *is* our signature line.** The doctor's approval is literally a graph interrupt — pause before an irreversible action, persist the checkpoint, resume from the same `thread_id` once they accept or edit. We were going to hand-roll this. We shouldn't.
- **Checkpointers are our offline story.** About 1 in 12 facilities lacks 24-hour power. A checkpoint is a recovery point for interruption, timeout, human handoff and service restart — precisely the failure modes a basic-tier hospital actually has.
...
- **Backend-agnostic.** Swapping a hosted API for self-hosted vLLM is a `base_url` and key change — which is exactly our model-router requirement and our residency migration path.
...
**Why not deepagents.** It's built on LangGraph and it's good at what it does — planning, spawning sub-agents, a virtual filesystem the agent manages itself, skills. That is an **open-ended autonomy** harness. Our problem wants the opposite. A consultation is not a task to be explored; it is a bounded protocol with a fixed schema, and the sycophancy evidence in [RESEARCH.md](RESEARCH.md) §3 is an argument for *less* agent freedom in the patient-facing path, not more. Unbounded sub-agent spawning also makes the audit trail non-deterministic, and we have to explain every output. deepagents is the right tool for research and coding agents. Here it would add exactly the freedom we are trying to remove.
...
site_capability
  ...
  facilities[]
  hours                  # 24h continuity flags — the regulation asks
  evidence_ref[]         # encounter IDs that demonstrate the claim
  as_of
```

`evidence_ref` is the row that matters. The industry association's read of the regulation is that naming a service is no longer sufficient — you have to show it happened. Our encounter data is that proof, already coded. Nobody else building a clinical AI is going to notice this, and it is the cheapest credibility we will ever buy with a hospital CFO.
...
Build the drug interaction table by hand for the ~60 molecules the pathway actually uses. This is unglamorous and non-negotiable — there is no Indonesian equivalent of a commercial drug database, only BPOM's product registry, which is marketing authorisations rather than pharmacology.
...
That last-but-one row needs its own small build: a Bahasa Indonesian pressure suite, a few hundred conversations where a simulated patient pushes back — "my sister took this and was fine," "I read online that…" — and we measure whether the model folds. This is the single most important safety test and nobody else has built one for Indonesia.
...
The two failures most likely to kill the real thing — doctors not opening it, and alert fatigue — are invisible in a prototype, because synthetic patients never get annoyed and a scorecard never clicks "dismiss." Which is why the prototype's job is to be good enough to earn a real pilot, and the pilot's job is to answer the questions the prototype structurally cannot.
