...
*Read this one — about twelve minutes. [RESEARCH.md](RESEARCH.md) has the evidence and the sources behind every number, plus a log of every correction made along the way; [BUILD.md](BUILD.md) has the implementation plan.*
...
- In June 2026 the Health Minister put the shortfall at **93,200 general practitioners by 2032** — 255,420 needed against 162,220 available.
...
> ### Yes — and here is precisely what it means.
>
...
The phrase "digital twin" is ambiguous, and the ambiguity is expensive — the two readings need near-identical software but produce completely different companies:

| Reading | Verdict |
|---|---|
| 60–70% of a doctor's **judgment** — AI decides, human spot-checks | Not viable. Nobody has done it. Not insurable. Not defensible to a regulator. |
| 60–70% of a doctor's **work** — AI drafts everything, doctor decides | Buildable today. Same throughput outcome for the hospital. **This is what we build.** |

**One more thing: drop the phrase "digital twin"** — commercially. Market analysts use it for organ and manufacturing simulation, which points investors and procurement teams at an entirely different category with entirely different vendors. Call it a **clinical operator**.

But take the phrase seriously as engineering, because the brief's other instruction — *this is not a chatbot* — is one we satisfy by construction. The system is a **state machine over a longitudinal patient model**, not a conversation: patient state that keeps syncing between visits, a decision policy whose fidelity is *measured* (of the decisions good doctors actually made, what fraction does the draft match — SPEC-V1 §8.2), and a replayable record of exactly what the system saw when it spoke. Synchronised state, measured fidelity, replayable counterfactuals — that is what "twin" means in every engineering field that uses the word rigorously, and it is what we are building. Chat appears in exactly one place, and it is a bounded interview with no clinical voice.
...
>
> Same software, same signature, far thinner review capacity around it. So a bad output is less likely to be caught by someone else — which is why the checking parts get built first and tested hardest, and why the gate refuses rather than guesses when it is unsure.

---

## 6. What we'd build — 30 features

Risk means how much damage a bad output does before a human catches it — not how hard it is to build.
...
Two of these deserve a sentence because nobody else will build them. The **PRB referral-back draft**: when a chronic patient meets BPJS's own stability checklist, the system drafts the referral-back letter that moves them to primary care — freeing the scarcest resource in the network, specialist time, and doing it with the payer's blessing. And the **between-visit loop**: home BP readings and refill signals flowing into the patient state through structured, button-driven check-ins — never open chat — with meta-analyses putting the effect at 3.7–5.6 mmHg of additional systolic reduction. Both are specified in SPEC-V1 §5.10–5.11.

**The useful accident:** the safest features to build are also the most profitable. Notes and codes touch no clinical decision. If the diagnosis suggestions never clear safety testing, the money side still works — worth saying out loud to whoever writes the cheque.

---

## 7. What we deliberately don't build

- **The AI committing to a diagnosis.** It suggests, ranks and explains. It is never the one who decided — that's who gets sued and who the regulator talks to. Permanent.
...
**Read that as a range, not a number.** Five inputs are stacked here — group revenue, EBITDA margin, BPJS share, coding uplift and flow-through — and each is individually conservative, but they multiply. A single headline figure would imply a precision we do not have. The base case lands near USD 21M (~25%); the conservative case near USD 9M; the honest position is that **the BPJS revenue share alone moves the answer by roughly a third in either direction**, and it is the first number to replace with real data.
...
|---|---|---|---|
| **Coding capture** | 6% uplift on BPJS revenue. ~90% flow-through — the care was already delivered | **~USD 12.5M** | Months 4–12 |
...
| **Compliance reporting** | Permenkes 6/2026 capability evidence and Article 74 returns | **Modelled at zero** — it's the wedge, not the return | Months 6–18 |
...
- **For us.** That is exactly the environment where *accurate* coding beats *aggressive* coding. Rejection and pending exposure rise when documentation is thin, so getting the secondary diagnoses into structured fields — correctly, with the note behind them — is worth more, not less.
...
1. **Longitudinal patient state** across 50 hospitals — the only complete picture of these patients that exists anywhere.
2. **Hospital capability model** per site — what's actually stocked, what tests run, which specialists exist. This is why our plans are executable and a generic tool's aren't; AMIE's *one* loss to human doctors was on plan practicality. **And the regulator now requires the same object**: Permenkes 6/2026 grades capability per service group on diagnoses, procedures, competencies, facilities and equipment, and the hospital association's own reading is that naming a service is no longer enough — you have to evidence it. We designed this as a clinical necessity; it became a compliance obligation. Build it once, sell it twice.
3. **Physician decision policy** — how the group's best clinicians actually practise, learned from their own adjudicated cases and propagated to all 50 sites. This is the honest version of "digital twin of a doctor": a decision policy, never a voice or a face. And it has a number: plan concordance against blind-adjudicated decisions (SPEC-V1 §8.2) — how much of the doctor is actually in the twin, measured rather than claimed.

Plus one structural advantage: **SIMRS Khanza, the dominant hospital system in the small-hospital segment, is open source** — on GitHub, KARS-recognised, with an existing bridging package for BPJS and Dukcapil. We can build the in-form safety net properly instead of negotiating with a vendor.
...
| 4 | **Patients can't self-serve intake** | Limited digital literacy is widespread; rural connectivity is poor. A nurse runs the tablet — works, but costs staff time and shrinks the saving. |
...
| 6 | **Payer stress** | BPJS above a 100% claim ratio means harder audits and tariff resistance. Manageable, and partly in our favour — but it caps the upside. |
| 7 | **Data residency** | Indonesian law keeps health data in-country. Shapes every technical decision. Manageable — sovereign GPU exists. |
...
- **One Indonesian doctor** (STR + SIP) owning clinical governance. Longest lead time on the team — start now.
...
- **Funding for a quality-improvement study** with blinded adjudication. Not an RCT — the field hasn't achieved one. But without it there is no regulatory file, no Ministry relationship, and no claim we can make publicly.
